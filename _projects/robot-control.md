...
caption: Low-latency VR system for remote robotic arm manipulation
description: >
...
---
This project explores remote robot manipulation through virtual reality and inverse kinematics. It spans two tracks: real-time mimicry control and data-driven motion playback—both of which informed my later work on hand interaction fidelity in Microsoft Mesh.

## Mimicry Control

In the first phase, I built a VR system that lets users drive robotic arms with natural hand and arm gestures. ROS data flows between Unity and the robot over lightweight sockets, keeping latency low enough for precise manipulation.
...
When COVID-19 closed the lab, I pivoted to a data-replay pipeline. The interpreter ingests timestamped joint angles from spreadsheets and generates Unity animation clips that faithfully reproduce captured robot motion.
...
- Demonstrated that low-bandwidth data exports are sufficient to recreate believable robot motion inside Unity—insights that later guided my remote collaboration prototypes at UW Graphics Group.
